...
import util.{ApiExtractor, ResourceExtractor, CoreUtils}
...
object validation {
  ...
  def validate(doc: ResourceListing, apis: List[ApiListing], fix: Boolean = true) =
    new SwaggerSpecValidator(doc, apis, fix).validate()
